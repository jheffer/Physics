...
while (N_std > 0 and stop ==0 and noise ==  False ):
  x = Last_frame
  while (x > Laser_off_frame and stop ==0):
	if (mod_array[x] > (mean + N_std* standard_deviation)):
	    stop = 1	
	    flash_frame = x
	    #print ("trigger 1: above mean")
	    if (mod_array[x-1] > (mean + N_std*standard_deviation) or (mod_array[x-1] - mean) > 0.7*(mod_array[x] - mean) ):
	      #print mod_array[x] 
	      #print mod_array[x-1] 
	      #if (mod_array[x-1] > (mean + N_std*standard_deviation)):
		  #print ("trigger 2 on above mean")
	      #if ((mod_array[x-1] - mean) > 0.7*(mod_array[x] - mean)):
		  #print ("trigger 2 70% of peak")
	      flash_frame = x-1
	x = x  -1
	N_std = N_std - 0.0001
	#print(N_std)
	#print(mean + N_std* standard_deviation)
if(noise ==  False):
  #print ("flash_frame")  	  
  print (flash_frame + Start_frame)
  #print(N_std)     
