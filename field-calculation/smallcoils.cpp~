...
// Small cube near the MOT region
...
int main()
{
  ...
  const int N = 20*1; // number of turns
  const double I = I0*N; // effective current after N turns
  const double S = 0.20; // coil spacing/m

  // data range
  const int probe_points = 100;
  const double probe_start = -0.3;
  const double probe_end = +0.3;
