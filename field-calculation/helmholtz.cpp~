...
int main()
{
  ...
  for (int i(0); i < circle_points; i++) circleX1.push_back(vect(-0.5*R, R*cos(2*Pi*double(i)/circle_points), R*sin(2*Pi*double(i)/circle_points)));
  for (int i(0); i < circle_points; i++) circleX2.push_back(vect(+0.5*R, R*cos(2*Pi*double(i)/circle_points), R*sin(2*Pi*double(i)/circle_points)));
  ...
  for (int i(0); i < probe_points; i++)
  {
    for (int j(0); j < probe_points; j++)
    {
      for( int k(0); k < probe_points; k++)
      {
        ...
        for (int i(0); i < circle_points; i++) B.AddCurrent(currX2[i], integrating_factor);
