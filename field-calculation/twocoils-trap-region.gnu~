set title "Coil pair, 5 cm radius, 20 cm spacing, 1 turn, 1 A (y = 0, z = 0)"
set xlabel "Position x/m"
set ylabel "Magnetic field strength/mG"

Bscale = 1E3*1E4 # convert T => mG

set output "twocoils-trap-region.ps"
set size 1.0, 1/1.61803398875
set terminal postscript portrait enhanced solid lw 1 "CM Roman, Times New Roman, Serif" 16

x1 = -0.10; x2 = +0.10; R = 0.05
# coil positions
set arrow from x1,graph 0 to x1, graph 1 nohead lt rgb "#999999"
set arrow from x2,graph 0 to x2, graph 1 nohead lt rgb "#999999"
# coil pos +/- coil radius
set arrow from x1-R,graph 0 to x1-R, graph 1 nohead lt rgb "#cccccc"
set arrow from x1+R,graph 0 to x1+R, graph 1 nohead lt rgb "#cccccc"
set arrow from x2-R,graph 0 to x2-R, graph 1 nohead lt rgb "#cccccc"
set arrow from x2+R,graph 0 to x2+R, graph 1 nohead lt rgb "#cccccc"

delB = "B(x = 0) = 21.3 mG\nB(x = 5 mm) = 21.5 mG\n{/Symbol D}B/B = 0.2/21.3 = 0.9%"
set label delB at graph 0.05, graph 0.80

# Plot only where y=0 and z=0
p\
"twocoils-trap-region.txt" u 1:(($2==0 & $3==0) ? (sqrt($4**2+$5**2+$6**2)*Bscale):NaN) t "|B|" pt 7 ps 0.2,\
"twocoils-trap-region.txt" u 1:(($2==0 & $3==0) ? ($4*Bscale):NaN) t "B_x" pt 7 ps 0.2 lt rgb "red",\
"twocoils-trap-region.txt" u 1:(($2==0 & $3==0) ? ($5*Bscale):NaN) t "B_y" pt 7 ps 0.2 lt rgb "green",\
"twocoils-trap-region.txt" u 1:(($2==0 & $3==0) ? ($6*Bscale):NaN) t "B_z" pt 7 ps 0.2 lt rgb "blue"
