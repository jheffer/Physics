...
// Large solenoid around the MOT region inside existing coils
...
{
  // coil properties
  const double a(0.83); // square side size/m
  const double S = 0.3; // coil spacing/m
  ...
  coil0.push_back( vect(-S/2, +a/2, +a/2) ); // square, sides a, in y-z plane
  coil0.push_back( vect(-S/2, -a/2, +a/2) );
  coil0.push_back( vect(-S/2, -a/2, -a/2) );
  coil0.push_back( vect(-S/2, +a/2, -a/2) );
 
  coil1.push_back( vect(0, +a/2, +a/2) );
  coil1.push_back( vect(0, -a/2, +a/2) );
  coil1.push_back( vect(0, -a/2, -a/2) );
  coil1.push_back( vect(0, +a/2, -a/2) );
  ...
  for (int i(0); i < coil2.size() - 1; i++) curr2.push_back(current(coil2[i], coil2[i+1], I));
  curr2.push_back(current(coil2[coil2.size()-1], coil2[0], I));
...
//        X.setVect(probe_start+i*probe_step, probe_start+j*probe_step, probe_start+k*probe_step); // calculate current position
        ...
        for (int i(0); i < curr2.size(); i++) B.AddCurrent(curr2[i], Nint);
